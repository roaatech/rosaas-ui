...
const TenantForm = ({
  ...
  useEffect(() => {
    if (formik.values.isAutoGenerated) {
      let uniqueNameTitle = formik.values.title
        .toLowerCase()
        .replace(/\s+/g, '-')
        .replace(/[^a-zA-Z0-9_-]/g, '')
      if (uniqueNameTitle) {
        formik.setFieldValue('uniqueName', uniqueNameTitle)
        setTitleToUnique(uniqueNameTitle)
        console.log(titleToUnique)
      } else {
        const randomUniqueName = generateRandomUniqueName()
        setTitleToUnique(randomUniqueName)
        formik.setFieldValue('uniqueName', randomUniqueName)

        console.log(titleToUnique)
      }
    }
  }, [formik.values.title, formik.values.isAutoGenerated])
          ...
          {type === 'create' && (
            <div className="mb-3">
              ...
              <div className="small p-1">
                <span className="mr-2">
                  <FormattedMessage id="Auto-Generated" />{' '}
                </span>
                <span className="small">
                  {' '}
                  <FontAwesomeIcon
                    icon={
                      formik.values.isAutoGenerated ? faToggleOn : faToggleOff
                    }
                    className={
